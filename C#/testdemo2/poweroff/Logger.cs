...
namespace poweroff
{
    ...
    public class Logger2
    {
        ...
        private class QueueManager : IDisposable
        {
            ...
            /// 日志文件保存的路径  
            /// </summary>  
           private static readonly string Path = BasePath + "\\AppLog\\";
            /// <summary>
            ...
            private void WriteToFile(Msg msg)
            {
                try
                {
                    ...
                    if (_writer != null)
                    {
                        _writer.WriteLine(string.Format("时间：{0}", msg.Datetime) + " 级别：" + msg.Type + "  " + msg.Location + "日志内容：" + msg.Text);
                        _writer.Flush();
                    }
                }
                catch (Exception e)
